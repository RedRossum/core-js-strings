...
function removeLastOccurrences(str, value) {
  const regexp = new RegExp(`${value}`, 'y');
  regexp.lastIndex = str.lastIndexOf(value);
...
function isPalindrome(str) {
  const matchStr = str.replace(/[^a-z]/gi, '').toLowerCase();
  return matchStr.split('').reverse().join('') === matchStr;
}
...
function invertCase(str) {
  return str.split('').reduce((rez, char) => {
    return char.toLowerCase() === char
      ? rez + char.toUpperCase()
      : rez + char.toLowerCase();
  }, '');
}
...
function extractNameFromTemplate(value) {
  const [, firstName, lastName] = value.replace(/[^a-z ]/gi, '').split(' ');
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) => output[input.indexOf(char)])
    .join('');
}
